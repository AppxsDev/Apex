...
class SendMessageToAiUseCase(private val aiRepository: AiRepository) {
    suspend operator fun invoke(message: String, conversationId: Long, history: List<Message> = emptyList()) : Result<Message> {
        return aiRepository.sendMessage(message, conversationId, history)
    }
}
